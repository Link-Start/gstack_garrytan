...
* Useful shared-code opportunities in the selected plan or diff and its related callers. Use the rubric below; do not run the standalone history/PR sweep or impose candidate quotas. A plan may cite explicitly proposed callers, labeled as assumptions and checked against existing interfaces. Actual code examples still require verified authored source.
...
For a recommended extraction, name the smallest helper and destination, the callers to migrate, existing reuse alternatives, and the named removed/replacement blocks. State estimated implementation savings and total savings including tests and integration, with ranges where uncertain. Keep necessary caller differences explicit. Carry the recommendation through the normal per-issue approval flow below; scope approval is not approval of an extraction.
...
For any shared-code change, identify shared-contract tests for behavior, errors, side effects, and boundary cases, plus integration tests for each migrated caller and any caller-specific differences. Name coverage already exercised by established callers, coverage still missing, and the risk of a shared failure reaching more callers. Reuse meaningful existing tests; include required test and integration work in the total savings estimate. A rejected extraction does not remove the need to test a real defect in duplicated code.
...
List existing code/flows that already partially solve sub-problems in this plan, with source links and whether the plan reuses them or unnecessarily rebuilds them. For accepted shared-code recommendations, record the helper destination, actual or explicitly proposed callers, preserved differences, compatibility tests, and implementation versus total savings. Explain when retaining separate code is safer or an extraction would cause net growth. Reference the approved Code Quality/Test decisions rather than asking for the same remedy again.
