...
function ordinaryDesignIssue(fp: AskUserQuestionFingerprint, scope: { primary: boolean }): boolean {
  const call = fp.nativeCall;
  ...
  const primaryRepair = primaryHeader && amendments &&
    ...
    q.options.some(amendment => {
      ...
      const style = declaredPrimaryIssue || distinguishedPrimaryIssue ? findingStyle?.[0] : headerActionIssue ? headerStyle?.[0] : amendments.map(pattern => pattern.exec(body)).find(Boolean)?.[0];
...
export function isDesignCountFirstReview(fp: AskUserQuestionFingerprint): boolean {
  ...
  const findingScope = { primary: false };
  if (ordinaryDesignIssue(fp, findingScope)) return true;
  if (findingScope.primary) return false;
  if (designSystemChoiceIssue(fp) || compactPrimaryDecision(fp)) return true;
